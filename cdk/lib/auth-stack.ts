...
//         `arn:aws:iam::${this.account}:oidc-provider/https://securetoken.google.com/${props.projectId}`
//       ],
...
export class AuthStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: AuthStackProps) {
    ...
    // Create an IAM OIDC provider for Firebase Authentication
    // This establishes the trust relationship with Firebase's authentication service
    const oidcProvider = new iam.OpenIdConnectProvider(this, 'FirebaseOidcProvider', {
      url: `https://securetoken.google.com/${props.projectId}`,
      clientIds: [props.projectId]
    });
    ...
    this.identityPool = new cognito.CfnIdentityPool(this, 'IdentityPool', {
      ...
      // Connect to Firebase Auth as an OpenID Connect provider
      // Use the ARN from the OIDC provider we just created
    ...
    // Export values that will be needed by the frontend application
